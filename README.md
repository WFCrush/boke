...
在后台打开文章编辑器后，有两种方式上传 ZIP 附件：

1. 点击编辑器工具栏里的 `ZIP` 按钮，选择 `.zip` 文件。
2. 直接把 `.zip` 文件拖到文章编辑器里。

上传成功后，后台会自动在正文里插入下载链接，类似：

```markdown
[文件名.zip](/boke/uploads/文件名.zip)
```

注意：左侧的 `导入 .md / .zip` 用于导入“Markdown 文章包”，普通 ZIP 附件请用编辑器里的 `ZIP` 按钮。
