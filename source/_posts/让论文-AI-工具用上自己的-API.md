...
tags:
  ...
  - FastAPI
  - Electron
description: 一款论文写作工具把模型供应商锁死在官方中转站，前端连个填 URL 的框都不给。这篇记录一次完整的「测绘 → 定位瓶颈 → 注入补丁 → 修掉衍生 bug」，顺带七条给后来者的铁律。
---

> 作者按：最近在折腾一款叫 Modex-MH-Agent 的桌面论文写作工具。它本质是个「AI 流水线」，把选题、检索、写作、配图、LaTeX 编译串起来，省去了大量手工活。但它有个让我极度不爽的设计——**模型供应商被锁死在官方中转站**，想用自己申请的 API Key 和中转地址？没门。前端连个填 URL 的框都不给你。
>
